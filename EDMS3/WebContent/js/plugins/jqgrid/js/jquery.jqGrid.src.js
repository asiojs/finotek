...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		$("<div class='loading ui-state-default ui-state-active' id='load_"+this.id+"'>"+this.p.text+"</div>").insertBefore(gv);
		...
		$(grid.bDiv)
		//class 부분이 추가되었습니다
			.append($('<div class="ui-jqgrid-bdiv-subwrapper" style="position:relative;'+(isMSIE && $.jgrid.msiever() < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
	...
	progressBar : function ( p ) {
		...
		return this.each(function(){
			...
			if(p.htmlcontent !== "") {
				...
				// exsoft.contextRoot
				//$("#load_"+$.jgrid.jqID(this.p.id)).html("<img src='/edms/img/icon/ajax-loader.gif' alt='' title=''><p>데이터 로딩 중입니다.<br>잠시만 기다려 주세요</p>");// p.htmlcontent
				$("#load_"+$.jgrid.jqID(this.p.id)).html("<img src='"+exsoft.contextRoot+"/img/icon/ajax-loader.gif' alt='' title=''><p>데이터 로딩 중입니다.<br>잠시만 기다려 주세요</p>");// p.htmlcontent
			}
			switch(p.loadtype) {
				case "disable":
					break;
