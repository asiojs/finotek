...
<div class="cnt_wrapper" id="workDocListBody">
	...
	<div class="contents">
		<div class="cnts_srch_wrapper">
			...
			<div class="cnts_srch_detail">
				<div class="cnts_srch">
					...
					<button type="button" class="srch_btn" onclick="workDocList.event.searchDocument();"></button>
				</div>
				<jsp:include page="/jsp/popup/docDetailSearch.jsp" /><!-- 상세검색 Layer 공통영역으로 분리 -->
				<div id="workDocListNoty"></div>
			</div>
				...
				<div class="tbl_extFunction">
					...
					<div class="extFunction_dropDown_wrapper hide">
						<ul class="extFunction_dropDown_menu">
							...
							<li><a href="#" class="tempbox" onclick="documentListLayerWindow.event.documentTempwork();">작업카트 추가</a></li>
						</ul>
					</div>
				</div>
			</div>
		...
		<div class="cnts_tbl_wrapper" id="workDocListTarget">
			<div class="cnts_list"><table id="workDocList"></table></div>
		</div>
		...
		<div class="pg_navi_wrapper">
			<ul id="workDocPager" class="pg_navi">
				<!-- 문서목록 Pager -->
			</ul>
		</div>
	</div>
...
<!-- script add -->
<script type="text/javascript" src="${contextRoot}/js/workspace/workDocList.js"></script>
<script type="text/javascript">
jQuery(function() {
	//page 초기화
	workDocList.init.initPage("${pageSize}");
	exsoft.util.layout.topMenuSelect("${menuType}");
	
	// listLayerWindow 초기화
	documentListLayerWindow.gObjectID = "workDocList";
	documentListLayerWindow.gWorkType = workDocList.workType;
// 	documentListLayerWindow.init.initBindEvent();
	
	// 상세검색 초기화
	docDetailSearch.targetGridId = "#workDocList";
	exsoft.util.grid.gridResize('workDocList','workDocListTarget',20,0);
	
	// 알림 아이디 설정
	exsoft.notyDivId = "#workDocListNoty";
});
</script>